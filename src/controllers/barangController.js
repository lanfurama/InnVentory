...
const addBarang = [
  ...
  body('kodebarang').custom(async (value) => {
    ...
    if (dup) {
      throw new Error('stock.addFailCodeExists');
    }
    return true;
  }),
  body('namabarang').custom(async (value) => {
    ...
    if (duplicate) {
      throw new Error('stock.addFailNameExists');
    }
    return true;
  }),
  body('image').custom(async (value, { req }) => {
    ...
    if (req.file.size > maxSize) {
      throw new Error('stock.imageTooBig');
    }
    return true;
  }),
  async (req, res) => {
...
const updateBarang = [
  ...
  body('kodebarang').custom(async (value, { req }) => {
    ...
    if (value !== req.body.oldKode && dup) {
      throw new Error('stock.editFailCodeExists');
    }
    return true;
  }),
  body('namabarang').custom(async (value, { req }) => {
    ...
    if (value !== req.body.oldNama && duplicate) {
      throw new Error('stock.editFailNameExists');
    }
    return true;
  }),
  body('image').custom(async (value, { req }) => {
